...
addCommand({
    pattern: "^tiktok ?(.*)",
    access: "all",
    desc: "_*TikTok videolarını indirmenizi sağlar.*_",
    usage: global.handlers[0] + "tiktok <url>"
}, async (msg, match, sock, rawMessage) => {
    const query = match[1];
    if (!query) {
        if (msg.key.fromMe) {
            return await sock.sendMessage(msg.key.remoteJid, { text: "_❌ Lütfen bir video linki gönderin!_", edit: msg.key });
        } else {
            return await sock.sendMessage(msg.key.remoteJid, { text: "_❌ Lütfen bir video linki gönderin!_" }, { quoted: rawMessage.messages[0] });
        }
    }
    ...
    if (query.match(/^(https?\:\/\/)?(www\.tiktok\.com|vm\.tiktok\.com|vt\.tiktok\.com)\/.+$/)) {
        ...
        if (msg.key.fromMe) {
            await sock.sendMessage(msg.key.remoteJid, { text: "_⏳ Video indiriliyor.._", edit: msg.key });
        } else {
            var publicMessage = await sock.sendMessage(msg.key.remoteJid, { text: "_⏳ Video indiriliyor.._" }, { quoted: rawMessage.messages[0] });
        }
        ...
        if (tk?.result?.type == "video") {
            ...
            if (tk?.result?.type == "image") {
                ...
                if (msg.key.fromMe) {
                    return await sock.sendMessage(msg.key.remoteJid, { text: "_❌ Bu URL için sonuç bulunamadı!_", edit: msg.key });
                } else {
                    return await sock.sendMessage(msg.key.remoteJid, { text: "_❌ Bu URL için sonuç bulunamadı!_", edit: publicMessage.key });
                }
            }
        }
    } else {
